...
function DasherButton(button) {
  ...
  this.dashButton.on("detected", function() {
    ...
    if (origin_body == '') {
        origin_body = options.body.text
    }

    var status = getStatus()
    options.body.text = origin_body + status
...
function getStatus() {
  var status_filename = './aircon_status.txt'
  var fs = require('fs');
  var new_status = 'off';

  var now_status = fs.readFileSync(status_filename, 'utf-8')
  if (now_status.toString() != 'on') {
    new_status = 'on';
  }
  ...
  var fs = require('fs');
  fs.writeFile(status_filename, new_status, function (err) {
    console.log(err);
  });
